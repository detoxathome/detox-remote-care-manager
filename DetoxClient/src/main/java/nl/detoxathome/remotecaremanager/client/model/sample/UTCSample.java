package nl.detoxathome.remotecaremanager.client.model.sample;
...
/**
 * The base class for a sample table that stores samples with a UTC time. Sample
 * classes should always extend either this class or {@link LocalTimeSample
 * LocalTimeSample}.
 * 
 * <p>This base class only defines the user (user ID) and sample time.
 * Subclasses should define the sample value. The sample time is defined by a
 * UTC time and time zone. The UTC time is defined as a long value, so it's
 * possible to sort objects on that field. This is not possible with ISO time
 * strings with different time zones. Note that a time zone offset as specified
 * in an ISO time string changes not only when moving to another location, but
 * also when entering of leaving DST time. The time zone field should be defined
 * as a location such as Europe/Amsterdam and not as an offset, so it is not
 * affected by DST time.</p>
 * 
...
public class UTCSample extends Sample {
	 ...
	 */
	public UTCSample(String user, ZonedDateTime tzTime) {
		setUser(user);
		this.utcTime = tzTime.toInstant().toEpochMilli();
		this.timezone = tzTime.getZone().getId();
		setLocalTime(tzTime.format(LOCAL_TIME_FORMAT));
	}
