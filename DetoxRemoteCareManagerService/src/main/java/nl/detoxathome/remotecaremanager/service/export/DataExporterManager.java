...
import nl.detoxathome.remotecaremanager.service.DatabaseLoader;
import nl.detoxathome.remotecaremanager.service.model.DataExportRecord;
import nl.detoxathome.remotecaremanager.service.model.DataExportTable;
...
public class DataExporterManager {
	...
	public void startExport(String project, User user)
		...
		if (runner.error != null)
			throwDetoxClientException(runner.error);
	}
	...
	private void throwDetoxClientException(Exception ex)
			throws DetoxClientException, HttpClientException,
			ParseException, IOException {
		if (ex instanceof DetoxClientException)
			throw (DetoxClientException)ex;
	...
	private void resumeExporter(Database authDb, DataExportRecord export) {
		...
		} catch (DetoxClientException | HttpClientException |
	...
	private void doStartExporter(Database authDb, DataExportRecord export)
			throws DatabaseException, DetoxClientException,
	...
	private void tryStartExporter(DataExportRecord export)
		...
		DetoxClient client = openClient(export.getUser());
	...
	private DetoxClient openClient(String userId)
			throws DetoxClientException, HttpClientException,
			ParseException, IOException {
		...
		DetoxClient client = new DetoxClient(baseUrl);
