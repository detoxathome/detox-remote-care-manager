...
import nl.detoxathome.remotecaremanager.client.model.ListUser;
import nl.detoxathome.remotecaremanager.client.model.Role;
import nl.detoxathome.remotecaremanager.client.model.compat.*;
import nl.detoxathome.remotecaremanager.client.model.detox.DetoxOnsSignupResult;
...
import nl.detoxathome.remotecaremanager.service.mail.EmailSender;
import nl.detoxathome.remotecaremanager.service.mail.EmailTemplate;
import nl.detoxathome.remotecaremanager.service.mail.EmailTemplateCollection;
import nl.detoxathome.remotecaremanager.service.mail.EmailTemplateRepository;
import nl.detoxathome.remotecaremanager.service.model.UserTable;
import nl.detoxathome.remotecaremanager.service.model.*;
import nl.detoxathome.remotecaremanager.service.validation.ModelValidation;
...
public class UserController {
	...
	private String createUniqueTempEmail(UserCache userCache)
		...
		for (int i = 0; i < DETOX_EMAIL_ATTEMPTS; i++) {
			String email = UUID.randomUUID().toString().toLowerCase()
					.replaceAll("-", "") + "@temp.detoxrcm.local";
	...
	private DatabaseObject doSetUserLock(ProtocolVersion version,
		...
		nl.detoxathome.remotecaremanager.client.model.User inputUser = parsedInput.user;
	...
	private List<String> getUserModelFields(ProtocolVersion version) {
		if (version.ordinal() >= ProtocolVersion.V6_0_7.ordinal()) {
			return DatabaseFieldScanner.getDatabaseFieldNames(
					nl.detoxathome.remotecaremanager.client.model.User.class);
		} else if (version.ordinal() >= ProtocolVersion.V6_0_6.ordinal()) {
	...
	private static class ParsedSetUserInput {
		public nl.detoxathome.remotecaremanager.client.model.User user;
		public Set<String> setFields;
	}

	private SetUserEmailResult setUserEmail(User setUser,
			nl.detoxathome.remotecaremanager.client.model.User inputUser, String templateName)
			throws HttpException {
