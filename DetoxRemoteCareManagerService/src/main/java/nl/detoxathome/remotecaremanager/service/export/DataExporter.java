...
/**
 * A data exporter can download data from Detox. write it to a directory,
...
public abstract class DataExporter {
	 ...
	 */
	public DataExporter(String id, User user, DetoxClient client, File dir,
	...
	/**
	 * Cancels the data exporter. It closes the Detox client.
	 */
	public void cancel() {
		synchronized (LOCK) {
			if (status != DataExportStatus.IDLE &&
	 ...
	 * <li>{@link DetoxClientException DetoxClientException}</li>
	...
	private void runThread() {
		try {
			tryRunThread();
		} catch (DataExportException | DetoxClientException |
	...
	private void tryRunThread() throws DataExportException,
			DetoxClientException, HttpClientException, ParseException,
			IOException {
		FileUtils.mkdir(dir);
	...
	/**
	 * Reads the data from Detox and writes it to the export directory.
	...
	protected abstract void readData(List<User> users)
			throws DataExportException, DetoxClientException,
